...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'Vestiaire',
		{
			...
			align    = 'top-left',
			elements = {
				{label = _U('couturier_clothes_civil'), value = 'citizen_wear'},
				{label = _U('couturier_clothes_vine'), value = 'couturier_cayo_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'couturier_cayo_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'couturier_cayo_actions_menu'
...
function OpenCouturierCayoActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'couturier_cayo_actions_menu',
		{
			title    = 'Couturier Cayo',
			align    = 'top-left',
			elements = elements
		},
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'couturier_cayo', function(data, menu)
					menu.close()
				end)
			end
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'couturier_cayo', vehicleProps)
		...
		local elements = {
			{label = 'Véhicule de Travail',  value = 'rebel2'},
			{label = 'Véhicule de Livraison',  value = 'youga3'},
		}
...
function OpenPlaneSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				'default', GetCurrentResourceName(), 'plane_spawner',
				{
					title    = _U('plane_menu'),
					align    = 'top-left',
					elements = elements,
				...
				function(data, menu)
					...
					local planeProps = data.current.value

					ESX.Game.SpawnVehicle(planeProps.model, Config.Zones.PlaneSpawnPoint.Pos, 90.0, function(plane)
						ESX.Game.SetVehicleProperties(plane, planeProps)
						local playerPed = GetPlayerPed(-1)
						TaskWarpPedIntoVehicle(playerPed,  plane,  -1)
					end)

					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'couturier_cayo', planeProps)
				...
				function(data, menu)
					...
					CurrentAction     = 'plane_spawner_menu'
					CurrentActionMsg  = _U('spawn_plane')
					CurrentActionData = {}
		...
		local elements = {
			{label = 'Avion de livraison',  value = 'velum2'},
		}
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				ESX.Game.SpawnVehicle(model, Config.Zones.PlaneSpawnPoint.Pos, 56.326, function(plane)
					local playerPed = GetPlayerPed(-1)
					TaskWarpPedIntoVehicle(playerPed,  plane,  -1)
				end)
			end,
...
function OpenMobileCouturierCayoActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_couturier_cayo_actions',
		{
			title    = 'Couturier Cayo',
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_couturier_cayo', 'couturier_cayo', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('a_couturier_cayo:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('a_couturier_cayo:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('a_couturier_cayo:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('a_couturier_cayo:putStockItems', itemName, count)
						end
...
AddEventHandler('a_couturier_cayo:hasEnteredMarker', function(zone)
	if zone == 'WoolFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo'  then
		CurrentAction     = 'wool_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementTissu' and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo'  then
		CurrentAction     = 'tissu_traitement'
		CurrentActionMsg  = _U('press_transform_tissu')
		CurrentActionData = {zone= zone}
	end		

	if zone == 'TraitementClothe' and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo'  then
		CurrentAction     = 'clothe_traitement'
		CurrentActionMsg  = _U('press_transform_clothe')
		CurrentActionData = {zone= zone}
	end	
		
	if zone == 'SellFarmCayo' and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo'  then
		CurrentAction     = 'farm_resell_cayo'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'SellFarmLosSantos' and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo'  then
		CurrentAction     = 'farm_resell_ls'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'CouturierCayoActions' and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo' then
		CurrentAction     = 'couturier_cayo_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end

	if zone == 'CouturierCayoCloackroom' and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo' then
		CurrentAction     = 'couturier_cayo_cloackroom_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo' then
	...
	if zone == 'VehicleSpawner2' and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo' then
		CurrentAction     = 'vehicle_spawner_menu2'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter2' and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo' then
	...
	if zone == 'PlaneSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo' then
		CurrentAction     = 'plane_spawner_menu'
		CurrentActionMsg  = _U('spawn_plane')
		CurrentActionData = {}
	end
		
	if zone == 'PlaneDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo' then
		...
		if IsPedInAnyVehicle(playerPed,  false) then
			...
			if distance ~= -1 and distance <= 1.0 then
				...
				CurrentAction     = 'delete_plane'
				CurrentActionMsg  = _U('store_plane')
				CurrentActionData = {vehicle = vehicle}
...
AddEventHandler('a_couturier_cayo:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'WoolFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo' then
		TriggerServerEvent('a_couturier_cayo:stopHarvest')
	end  
	if (zone == 'SellFarmCayo') and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo' then
		TriggerServerEvent('a_couturier_cayo:stopSell1')
	end
	if (zone == 'SellFarmLosSantos') and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo' then
		TriggerServerEvent('a_couturier_cayo:stopSell2')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.CouturierCayoActions.Pos.x, Config.Zones.CouturierCayoActions.Pos.y, Config.Zones.CouturierCayoActions.Pos.z)
		...
		AddTextComponentString("Couturier Cayo")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo' then

		for k,v in pairs(Config.Zones)do
			if v.Type == 2 then
				local blip2 = AddBlipForCoord(v.Pos.x, v.Pos.y, v.Pos.z)

				SetBlipSprite (blip2, 175)
	...
	if PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo' then
		...
		for k,v in pairs(Config.Zones)do
			if v.Type == 3 then
				...
				SetBlipSprite (blip3, 16)
				SetBlipDisplay(blip3, 4)
				SetBlipScale  (blip3, 0.85)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('a_couturier_cayo:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('a_couturier_cayo:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'wool_harvest' then
					TriggerServerEvent('a_couturier_cayo:startHarvest', CurrentActionData.zone)
					TaskStartScenarioInPlace(playerPed, 'WORLD_HUMAN_HANG_OUT_STREET', 0, false)
				end
				if CurrentAction == 'tissu_traitement' then
					TriggerServerEvent('a_couturier_cayo:startTransform1', CurrentActionData.zone)
					TaskStartScenarioInPlace(playerPed, 'WORLD_HUMAN_HANG_OUT_STREET', 0, false)
				end
				if CurrentAction == 'clothe_traitement' then
					TriggerServerEvent('a_couturier_cayo:startTransform2', CurrentActionData.zone)
					TaskStartScenarioInPlace(playerPed, 'WORLD_HUMAN_HANG_OUT_STREET', 0, false)
				end
				if CurrentAction == 'farm_resell_cayo' then
					TriggerServerEvent('a_couturier_cayo:startSell1', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell_ls' then
					TriggerServerEvent('a_couturier_cayo:startSell2', CurrentActionData.zone)
				end
				if CurrentAction == 'couturier_cayo_actions_menu' then
					OpenCouturierCayoActionsMenu()
				end
				if CurrentAction == 'couturier_cayo_cloackroom_menu' then
					OpenCouturierCayoActionsMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'couturier_cayo', vehicleProps)
					end
				...
				if CurrentAction == 'plane_spawner_menu' then
					OpenPlaneSpawnerMenu()
				end
				if CurrentAction == 'delete_plane' then
				
					if Config.EnableSocietyOwnedVehicles then
						local vehicleProps = ESX.Game.GetVehicleProperties(CurrentActionData.plane)
						TriggerServerEvent('esx_society:putVehicleInGarage', 'couturier_cayo', planeProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'couturier_cayo' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobileCouturierCayoActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)